var app = (function () {
    ...
    */
    ...
    class CorDashboardDonutChart extends Component {
      ...
      chart(chartData) {
        this.addTooltip();
        chartData.sort((a, b) => a.value < b.value ? 1 : -1); // Define size & radius of donut pie chart
        ...
        const arcText = d3.scaleOrdinal().range([0, width]); // Determine size of arcs

        const arc = d3.arc().innerRadius(radius - 50).outerRadius(radius - 10); // Create the donut pie chart layout

        const pie = d3.pie().value(d => d["value"]).sort(null);
        const tooltip = document.querySelector('.toolTip'); // Append SVG attributes and append g to the SVG

        const svg = d3.select(this.querySelector("svg")).attr("width", width).attr("height", height).append("g").attr("transform", "translate(" + radius + "," + radius + ")"); // Define inner circle

        svg.append("circle").attr("cx", 0).attr("cy", 0).attr("r", 100).attr("fill", "#fff"); // Calculate SVG paths and fill in the colours

        const g = svg.selectAll(".arc").data(pie(chartData)).enter().append("g").attr("class", "arc") // Make each arc clickable 
        .on("click", updateChart); // Append the path to each g

        g.append("path").transition().attr("d", arc).attr("fill", d => d.data.color);
        ...
        .enter().append('li').append('a').attr('href', d => `#${d.label}`).html(d => `<span>${d.label}:</span> <strong>${d.value}</strong>`).on("click", updateChart).append('span').style("background-color", d => d.color);
      }
    ...
    class CorDashboardBarChart extends Component {
      ...
      chart(data, target) {
        ...
        var height = 200;
        var yScale = d3.scaleLinear().domain([0, d3.max(data, d => {
          return d.total;
        })]).range([0, height]); // Transitions

        var t = d3.transition().duration(750).ease(d3.easeBackInOut); // Creation of the chart

        d3.select(target.querySelector(".chart")).attr('height', height).selectAll('div').data(data).enter().append('div').append('div').transition(t).style('height', d => yScale(d.total) + "px"); // Creation of the list

        d3.select(this).append('ol').classed('js-cor-dashboard-barchart__list', true).selectAll(null).data(data).enter().append('li').append('a').attr('href', d => `#${d.label}`).html(d => `<span>${d.label}</span> <strong>${d.total}</strong>`).append('span');
      }
                        ...
                        ${card.cardtype === 'multilinguismOverview' ? `
                            <div class="cor-dashboard-multilinguismTable">
                                ...
                                <div>
                                    <h2>Languages</h2>  
                                    <ol>
                                        ${card.multilingualContent[0].translations.map(translation => translation.label === "EN" ? `<li class="cor-dashboard__multilinguismTable__en"><a href="#">${translation.label}</a></li>` : `<li><a href="#">${translation.label}</a></li>`).join('')}
                                    </ol>
                                </div>
                      
                                ${card.multilingualContent.map(multilingualContent => `
                                        <div>
                                            <h2>${multilingualContent.contenttype} </h2>

                                            <ol>
                                                ${multilingualContent.translations.map(translation => translation.label === "EN" ? `<li class="cor-dashboard__multilinguismTable__en"><a href="#">${translation.total}</a></li>` : `<li><a href="#">${translation.total}</a></li>`).join('')}
                                            </ol>
                                        </div>
                                    `).join('')}
